						...
						<div className="p-5 border border-gray-200 dark:border-gray-700">
							...
							</p>
							{
								item.sub && item.sub.map((subItem: SubAccordion, index: number) => {
									const lastItem = item.sub ? item.sub.length - 1 : 0
									...
									return (
										<div key={ index } className={ index === 0 ? 'mt-8' : '' }>
											<h3>
												<button
													type="button"
													className={ (index === 0 ? 'rounded-t-xl ' : (!show[subItem.id] && (lastItem === index) ? 'rounded-b-xl ' : '')) + 'flex items-center justify-between w-full p-5 font-medium rtl:text-right text-gray-900 border border-gray-200 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-800 dark:border-gray-700 dark:text-gray-100 hover:bg-gray-100 dark:hover:bg-gray-800 gap-3' }
											...
											<div className={ show[subItem.id] ? 'block' : 'hidden' }>
												<div
													className={ (lastItem === index ? 'rounded-b-xl ' : '') + 'p-5 border border-gray-200 dark:border-gray-700' }>
													<p className="text-gray-800 dark:text-gray-200">
														{ subItem.ans }
													</p>
												</div>
