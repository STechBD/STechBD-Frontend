...
import type { Comparison, Currency, Feature, State } from '@/data/type'


/**
 * Renders a comparison table for features and pricing plans.
 *
 * @param { Feature[] } feature An array of features to be displayed in the table
 * @param { Comparison[] } data An array of comparison data for different pricing plans
 * @returns { JSX.Element } The Comparison Table component.
 */
export default function ComparisonTable({ feature, data }: { feature: Feature[], data: Comparison[] }): JSX.Element {
				...
				>
					...
					<div className="hidden lg:block sticky top-[3.5rem] start-0 py-2 bg-white dark:bg-slate-900">
						...
						<div className={ 'grid gap-6 grid-cols-' + column }>
							...
							{ data.map((item: Comparison): JSX.Element => {
								return (
									<div key={ item.title }>
										{ /** Header **/ }
										<div
											...
											<div className="text-center">
												...
												{ item.price ? (
													<p className="text-xs text-gray-500">
														{ symbol }{ item.price[currency] } { item.period === 0 ? 'per year' : 'per month' }
													</p>
												) : (
													<p className="text-xs text-gray-500">
														Contact for pricing
					...
					{ /** Section **/ }
					{ feature.map((feature: Feature): JSX.Element => {
						return (
							<div key={ feature.title } className="space-y-4 lg:space-y-0">
								{ /** List **/ }
								...
								<ul className="grid lg:grid-cols-6 lg:gap-6">
									...
									<li className="col-span-1 py-1.5 lg:py-3 px-4 lg:px-0 lg:text-center bg-gray-100 dark:bg-white/[.05]">
										<div className="grid grid-cols-6 lg:block">
											...
											<svg
												className="w-4 h-4 lg:mx-auto text-gray-500"
												xmlns="http://www.w3.org/2000/svg" width="16" height="16"
												fill="currentColor" viewBox="0 0 16 16"
											>
												<path fillRule="evenodd"
